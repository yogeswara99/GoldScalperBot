# ================================================================
#  backtest_engine_v1.0.py  —  GoldScalperBot  XAUUSD...
#  VERSION 1.0  —  $17,332 net profit | 73.3% win rate | 1,120 trades
#
#  Signal: Placeholder (H4 trend + H1 RSI + M15 Stochastic RSI)
#  SL/TP : ATR-based (1.5x M15 ATR)
#  Trade management: breakeven at 3.5x spread, ATR trail after
#  Session: No session close — trades run freely to SL/TP
#  Filters: Volume Z > 0, London delay 08:30, Monday 70% lot
#
#  DO NOT MODIFY — locked reference version
#  For experiments copy to backtest_engine.py and version it
# ================================================================
...
class TradeSimulator:
        ...
        for t in self.open_trades:
            ...
            if t.direction == "LONG":
                ...
                if sl_hit and tp_hit:
                    cp, reason = (t.tp_price,"TP") if bar["Close"]>=bar["Open"] else (t.sl_price,"SL")
    ...
    def _close_trade(self, t, cp, reason, ct):
        ...
        if pnl_usd<0: self.daily_loss+=abs(pnl_usd); self.session_losses+=1
        else: self.session_losses=0
...
class BacktestRunner:
    ...
    def _run_single(self, df_h4, df_h1, df_m15, params):
        ...
        for i in range(50, len(df_m15)):
            ...
            # Entries only during sessions — no session close
            if not in_session: continue

            # Filter 2: London open delay
            ...
            if len(h4_bars)<20 or len(h1_bars)<5: continue
...
if __name__=="__main__":
    print("="*55)
    print("  GoldScalperBot v1.0  —  XAUUSD...")
    print("="*55)
    print(f"  Symbol : {SYMBOL}  Account: ${ACCOUNT_BALANCE:,.2f}")
    print("="*55)
