# ================================================================
#  backtest_engine_v1.1.py  —  GoldScalperBot  XAUUSD...
#  VERSION 1.1  —  6 surgical drawdown fixes
#
#  Changes from v1.0:
#    1. Weekly DD circuit breaker now ENFORCED
#       (was defined in config but never checked)
#    2. Session trade limit now ENFORCED
#       (MAX_TRADES_PER_SESS = 5 per London/NY session)
#    3. SL/TP same-bar conflict: resolved by DISTANCE FROM OPEN
#       (v1.0 used candle direction — biased toward TP in uptrends)
#    4. H4 trend: EMA20/EMA50 + price above EMA20
#       (v1.0 used crude 5-bar close comparison)
#    5. Volume filter: Z >= 0.5 (config FILTER_VOL_MIN_Z raised)
#    6. Commission: $3.50/lot/side from config (v1.0 was $0)
#
#  Baseline (v1.0): $13,659 | 72.9% WR | 785 trades | 37.8% DD
#  Target  (v1.1): DD <= 28% | WR >= 70% | trades >= 500
#
#  Signal architecture UNCHANGED from v1.0
#  SMC engine integration planned for v1.2
#
...
SLIPPAGE_POINTS = 3
# COMMISSION_USD now sourced from config.py (v1.1)
...
class TradeSimulator:
    def __init__(self, starting_balance=ACCOUNT_BALANCE):
        self.balance = starting_balance; self.equity = starting_balance
        self.peak_equity = starting_balance
        self.open_trades: List[BacktestTrade] = []
        self.closed_trades: List[BacktestTrade] = []
        self.trade_counter = 0; self.daily_loss = 0.0
        self.session_losses = 0; self.equity_curve = [starting_balance]
        # v1.1: new tracking fields
        self.weekly_loss = 0.0            # weekly DD accumulator
        self.session_trade_count = 0      # trades opened in current session
        self.current_session_key = None   # identity key of current session
    ...
    def open_trade(self, direction, entry_price, sl_price, tp_price,
        ...
        if direction == "LONG":
            ...
            adj_sl= sl_price - sp; adj_tp = tp_price + sp
        else:
            adj_e = entry_price - SLIPPAGE_POINTS*POINT
            adj_sl= sl_price + sp; adj_tp = tp_price - sp
        t = BacktestTrade(
        ...
        self.session_trade_count += 1  # v1.1: track per-session count
        return t
        ...
        for t in self.open_trades:
            ...
            if t.direction == "LONG":
                ...
                if sl_hit and tp_hit:
                    # v1.1: resolve by distance from open — whichever
                    # level is closer to bar open was hit first
                ...
                if sl_hit and tp_hit:
                    # v1.1: resolve by distance from open
                    dist_sl = abs(bar["Open"] - t.sl_price)
                    dist_tp = abs(bar["Open"] - t.tp_price)
                    cp, reason = (t.tp_price,"TP") if dist_tp<=dist_sl else (t.sl_price,"SL")
                elif sl_hit: cp, reason = t.sl_price, "SL"
    ...
    def _close_trade(self, t, cp, reason, ct):
        pnl_pts = (cp-t.entry_price)/POINT if t.direction=="LONG" else (t.entry_price-cp)/POINT
        pnl_usd = pnl_pts * POINT * CONTRACT_SIZE * t.lot_size - COMMISSION_USD*t.lot_size*2
        t.exit_price=cp; t.close_time=ct; t.pnl_usd=round(pnl_usd,2)
        t.pnl_points=round(pnl_pts,1); t.exit_reason=reason; t.is_open=False
        self.balance+=pnl_usd; self.equity=self.balance
        self.peak_equity=max(self.peak_equity,self.equity)
        self.equity_curve.append(round(self.equity,2))
        if pnl_usd<0:
            self.daily_loss+=abs(pnl_usd)
            self.weekly_loss+=abs(pnl_usd)   # v1.1: accumulate weekly losses
            self.session_losses+=1
        else:
            self.session_losses=0
        self.open_trades.remove(t); self.closed_trades.append(t)

    def is_trading_allowed(self):
        if self.daily_loss >= MAX_DAILY_LOSS_USD:          return False, "Daily limit hit"
        if self.session_losses >= MAX_CONSEC_LOSSES:        return False, "Consec losses"
        if self.weekly_loss >= WEEKLY_DD_USD:               return False, "Weekly DD limit"   # v1.1
        if self.session_trade_count >= MAX_TRADES_PER_SESS: return False, "Session limit"     # v1.1
...
class ResultAnalyser:
    ...
    def analyse(sim, params):
        ...
        r.max_drawdown_usd=max_dd; r.max_drawdown_pct=(max_dd/ACCOUNT_BALANCE)*100
        if len(pnls)>1:
...
class BacktestRunner:
    def __init__(self):
        self.loader   = DataLoader(SYMBOL); self.scorer = ScoringEngine()
        self.simulator= TradeSimulator();   self.logger  = BacktestLogger()
    ...
    def _run_single(self, df_h4, df_h1, df_m15, params):
        ...
        # v1.1: H4 EMA20 + EMA50 for robust trend detection
        df_h4["EMA20"]    = Indicators.ema(df_h4["Close"], 20)
        df_h4["EMA50"]    = Indicators.ema(df_h4["Close"], 50)
        ...
        for i in range(50, len(df_m15)):
            ...
            # v1.1: Weekly reset on new ISO week
            ...
            # v1.1: Session trade counter — reset on new session window
            if in_london:
            ...
            if FILTER_LONDON_DELAY and hour==LONDON_OPEN_H and bar_time.minute<LONDON_OPEN_MIN:
                continue

            allowed, _ = self.simulator.is_trading_allowed()
            if not allowed: continue
            if len(self.simulator.open_trades)>0: continue

            # v1.1: need 50 H4 bars minimum for EMA50 accuracy
            h4_bars = df_h4[df_h4.index<=bar_time].tail(50)
            h1_bars = df_h1[df_h1.index<=bar_time].tail(20)
            if len(h4_bars)<50 or len(h1_bars)<5: continue

            h1_last = h1_bars.iloc[-1]

            # v1.1: H4 trend via EMA20/EMA50 + price confirmation
            # Replaces v1.0: h4_bars["Close"].iloc[-1] > h4_bars["Close"].iloc[-5]
            h4_ema20    = h4_bars["EMA20"].iloc[-1]
            h4_ema50    = h4_bars["EMA50"].iloc[-1]
            h4_close    = h4_bars["Close"].iloc[-1]
            h4_trend_up = (h4_ema20 > h4_ema50) and (h4_close > h4_ema20)

            h1_rsi   = h1_last.get("RSI14",50.0)
            h1_slope = h1_last.get("EMA20slope",0.0)
            h1_bull  = (h1_rsi>50) and (h1_slope>0)
            h1_bear  = (h1_rsi<50) and (h1_slope<0)

            stk_k = bar.get("StochK",50.0); stk_d = bar.get("StochD",50.0)
            prev  = df_m15.iloc[i-1]
            bull_cross = (stk_k>stk_d and prev.get("StochK",50)<=prev.get("StochD",50) and stk_k<80)
            bear_cross = (stk_k<stk_d and prev.get("StochK",50)>=prev.get("StochD",50) and stk_k>20)
            ...
            # v1.1: volume filter = FILTER_VOL_MIN_Z from config (0.5)
            if vol_z < FILTER_VOL_MIN_Z: continue

            spread_pts = bar.get("Spread",25) if not pd.isna(bar.get("Spread",np.nan)) else 25
            atr_pts    = atr_val / POINT
            sl_pts     = int(atr_pts * SL_ATR_MULTIPLIER)
            sl_pts     = max(int(atr_pts*SL_ATR_MIN_MULT), min(int(atr_pts*SL_ATR_MAX_MULT), sl_pts))
            ...
            self.simulator.open_trade(direction, entry, sl, tp, lot,
        ...
        if len(self.simulator.open_trades)>0:
            ...
            for t in list(self.simulator.open_trades):
                self.simulator._close_trade(t, last_bar["Close"],
                                             "BACKTEST_END", df_m15.index[-1])
...
if __name__=="__main__":
    print("="*55)
    print("  GoldScalperBot v1.1  —  XAUUSD...")
    print("="*55)
